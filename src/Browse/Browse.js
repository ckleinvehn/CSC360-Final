...
class Browse extends React.Component {
  ...
  changeQueryArgs = () => {
    this.props.history.push(
      `${window.location.pathname}?begin=${this.state.beginValue}&end=${
        this.state.endValue
      }&queries=${encodeURIComponent(this.state.queries.join())}`
    );
  };
    ...
    this.setState(
      {
        queries: this.state.queries
          .slice(0, i)
          .concat(this.state.queries.slice(i + 1)),
      },
      () => {
        this.changeQueryArgs();
  ...
  handleDateChange = (date, isStartDate) => {
    ...
    if (isStartDate && dateVal.isBefore(moment(this.state.endValue)))
      this.setState(
        { beginValue: dateVal.format("YYYY-MM-DD") },
        this.changeQueryArgs
      );
    else if (!isStartDate && dateVal.isAfter(moment(this.state.beginValue)))
      this.setState(
        { endValue: dateVal.format("YYYY-MM-DD") },
        this.changeQueryArgs
      );
    else {
      console.log(dateVal);
      console.log(moment(this.state.startValue));
  ...
  handleSubmit = () => {
    browseFetch(
      this.state.queries.map((query) => encodeURIComponent(query)),
      this.state.beginValue,
      this.state.endValue
    ).then((hits) => {
      this.setState({
        doNotDisplay: false,
      ...
      <main style={{ position: "relative" }}>
        <div style={Object.assign({ left: 0, flexDirection: "column" }, half)}>
          {this.state.errors.map((error, i) => (
            <Alert style={{ marginBottom: "1rem" }} severity="error" key={i}>
              {error}
            </Alert>
          ))}
          ...
          <div>
            <TextField
              inputProps={{ "aria-label": "search-term-textbox" }}
            ...
            <label style={label}>
              ...
              <input
                ...
                onChange={(event) => {
                  this.handleDateChange(event.target.value, true);
                }}
              />
            </label>
            ...
            <label style={label}>
              ...
              <input
                ...
                onChange={(event) => {
                  this.handleDateChange(event.target.value, false);
                }}
              />
            </label>
          ...
          <div style={{ position: "relative", right: 125, width: "45vw" }}>
            {this.state.doNotDisplay ? (
              <>
                ...
                <div
                  style={{ position: "absolute", top: "40%", left: "21.5%" }}
                >
                  <Typography variant="h5" align="center">
                    You can perform up to 10 queries at once.
                    <br />
                    Hit enter to lock in your choice.
                  </Typography>
                </div>
              </>
            ) : (
