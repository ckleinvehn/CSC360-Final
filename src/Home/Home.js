...
export default class Home extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <Typography variant="h3" align="center" style={heading}>
          <b>
    ...
    <>
      ...
      <Card
        ...
        className={clsx(
          ...
          { [classes.relativeFlow]: !props.opened }
        )}
        raised
      >
        <a
        ...
        <div
          className={clsx(
            { [classes.opened]: props.opened },
            { [classes.notOpened]: !props.opened }
          )}
        >
          <CardActions disableSpacing>
            <Button
              className={clsx(classes.readMore, {
                [classes.readMoreOpen]: props.opened,
              })}
              startIcon={<Link />}
              variant="outlined"
              href={props.article.url}
        ...
        <Collapse in={props.opened} timeout={0} unmountOnExit>
          <CardContent>
            {["descriptions", "organizations", "people", "geography"].map(
              (tags, i) =>
                props.article[tags].length > 0 ? (
                  <Box key={i} className={classes.tags}>
                    <Typography variant="h6">{tags}</Typography>
                    {props.article[tags].map((tag, j) => (
                      <Chip
                        label={tag[0]}
                        title={tag[1] || ""}
                        key={j}
